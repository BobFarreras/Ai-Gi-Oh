-- docs/supabase/sql/086_arena_opponents_guill_midutech.sql
...
--   - Midutech: rival comodín que puede aparecer aleatoriamente en cualquier nivel (rotación por código).
...
INSERT INTO arena_opponents (id, code_name, display_name, avatar_url, intro_url, story_opponent_id, sort_order) VALUES
  ...
  ('training-midutech','midutech','Midutech','/assets/story/opponents/opp-ch1-midutech/avatar-Midutech.webp','/assets/story/opponents/opp-ch1-midutech/intro-Midutech.webp','opp-midutech',8)
...
INSERT INTO arena_opponent_deck_variants (id, opponent_id, label, sort_order) VALUES
  ...
  ('midutech-offense','training-midutech','Midutech Offense',1),
  ('midutech-control','training-midutech','Midutech Control',2)
...
DELETE FROM arena_deck_variant_cards WHERE variant_id IN ('apex-annihilation','apex-lockdown','midutech-offense','midutech-control');
...
SELECT v.variant_id, c.card_id, v.zone, c.ord FROM (VALUES
  ...
  ('midutech-offense','DECK',ARRAY['entity-chatgpt','entity-gemini','entity-claude','entity-kali-linux','entity-deepseek','entity-python','entity-postgress','entity-nextjs','entity-react','entity-vercel','exec-fusion-gemgpt','exec-fusion-kaclauli','exec-direct-damage-900','exec-boost-atk-400','exec-framework-atk-300','exec-draw-1','trap-kernel-panic','trap-runtime-punish','trap-counter-intrusion','trap-atk-drain']),
  ('midutech-offense','FUSION',ARRAY['fusion-gemgpt','fusion-kaclauli']),
  ('midutech-control','DECK',ARRAY['entity-chatgpt','entity-gemini','entity-claude','entity-kali-linux','entity-deepseek','entity-python','entity-postgress','entity-nextjs','entity-react','entity-supabase','exec-fusion-gemgpt','exec-fusion-kaclauli','exec-llm-def-300','exec-heal-700','exec-framework-atk-300','exec-draw-1','trap-kernel-panic','trap-runtime-punish','trap-counter-intrusion','trap-def-fragment']),
  ('midutech-control','FUSION',ARRAY['fusion-gemgpt','fusion-kaclauli'])
