-- docs/supabase/sql/057_phase_analytics_card_type.sql - Añade el tipo de carta (Entidad/Ejecución/Trampa/Fusión) a los rankings de cartas usadas/compradas para que sean reconocibles en el panel.
begin;
...
as $$
  select jsonb_build_object(
    ...
    'topCardsUsed', coalesce((
      select jsonb_agg(jsonb_build_object('cardId', c.card_id, 'cardName', coalesce(cc.name, c.card_id), 'cardType', cc.type, 'count', c.cnt) order by c.cnt desc)
      from (
    ...
    'topCardsPurchased', coalesce((
      select jsonb_agg(jsonb_build_object('cardId', x.card_id, 'cardName', coalesce(cc.name, x.card_id), 'cardType', cc.type, 'count', x.cnt) order by x.cnt desc)
      from (
