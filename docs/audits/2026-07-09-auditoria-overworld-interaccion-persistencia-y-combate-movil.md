...
- **Fase D (#1) — COMPLETADA (revisada).** La columna de atributos del desktop se extrae SIN cambios a un componente compartido `components/game/card/internal/CardHologramStatColumn.tsx` (mismos tamaños `text-6xl`/iconos `w-12`), usado por los modos `full` (desktop, intacto) y `lite` (móvil). El móvil pasa a mostrar esa MISMA columna, centrada sobre la carta y grande/legible (antes se veía un badge plano/pequeño). Sin `blur` de GPU ni animación en bucle. Con tests. Verificado: `tsc` + `eslint` + tests de card. **Nota:** el modo `lite` se activa también en desktop si el dispositivo se detecta como limitado o hay `prefers-reduced-motion`; conviene comparar con `pnpm perf:baseline:mobile`.
- **Fase C (#3) — COMPLETADA.** Decisión tomada: `claim-reward` devuelve la `ICard` otorgada (vía `loadCardsByIds`), sin segunda petición. Nuevo overlay `components/hub/story/overworld/hud/OverworldCardPickup.tsx` que muestra el componente `Card` real a tamaño de lectura y lo encoge hacia el jugador (con failsafe). El motor añade `markObjectCollected` (oculta el nodo y libera su celda sin animación de canvas, porque la anima React). Con tests. Verificado: 2 tests + `tsc` + `eslint`. **Nota:** la trayectoria de encogido cae hacia abajo (no al píxel exacto del jugador); conviene verlo en móvil y afinar.
