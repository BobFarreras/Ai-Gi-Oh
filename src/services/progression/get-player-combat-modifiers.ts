...
// (ficha 8) del jugador, para aplicarlos en la preparación de partidas PvE (Story/Arena/Survival). No-fatal: si
...
export async function getPlayerCombatModifiersByPlayerId(playerId: string): Promise<IPlayerCombatModifiers> {
  if (!playerId.trim()) return EMPTY;
  try {
    const client = await createSupabaseServerClient();
    const modifiers = await new GetPlayerSkillModifiersUseCase(new SupabaseSkillTreeRepository(client)).execute(playerId);
    return {
...
export async function getPlayerCombatModifiers(): Promise<IPlayerCombatModifiers> {
  ...
  return getPlayerCombatModifiersByPlayerId(session.user.id);
}
