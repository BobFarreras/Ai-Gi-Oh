...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!firstVictory) return NextResponse.json({ duelProgress, rewarded: false }, { status: 200, headers: response.headers });
    ...
    return NextResponse.json(
      { duelProgress, rewarded: true, rewardNexus: duel.rewardNexus, rewardPlayerExperience: duel.rewardPlayerExperience, rewardCardIds, playerProgress },
