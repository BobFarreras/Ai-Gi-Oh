  ...
  return (
    <>
      <div className="grid min-h-0 flex-1 gap-3 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <div className="home-modern-scroll min-h-0 flex-1 overflow-y-auto overflow-x-hidden pr-2">
            <div className="grid grid-cols-[repeat(auto-fill,minmax(88px,1fr))] gap-3 justify-items-center pb-4">
              {filteredCards.map((entry) => {
                ...
                return (
                  <button key={entry.id} type="button" aria-label={`Seleccionar ${entry.name} en market`} className="relative w-[88px]" onClick={() => { onSelectCard(entry.id); setIsMobileDetailOpen(true); }}>
        ...
        <div className="hidden min-h-0 xl:block">{detailContent}</div>
      </div>

      <AdminMobileDetailDialog isOpen={isMobileDetailOpen} onClose={() => setIsMobileDetailOpen(false)} closeAriaLabel="Cerrar detalle de listing">
        {detailContent}
