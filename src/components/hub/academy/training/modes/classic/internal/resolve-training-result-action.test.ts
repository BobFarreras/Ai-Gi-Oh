// src/components/hub/academy/training/modes/classic/internal/resolve-training-result-action.test.ts - Valida la CTA post-combate clásica.
...
describe("resolveTrainingResultAction", () => {
  it("sugiere jugar el siguiente tier cuando acaba de desbloquearse", () => {
    ...
    expect(result.href).toBe("/hub/academy/training/arena/classic?tier=3");
  });
  ...
  it("vuelve a selección cuando no hay tier nuevo", () => {
    ...
    expect(result.href).toBe("/hub/academy/training/arena/classic?tier=4");
  });
});
