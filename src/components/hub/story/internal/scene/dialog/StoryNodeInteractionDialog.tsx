...
export function StoryNodeInteractionDialog({
  ...
  return (
    <AnimatePresence>
      {isOpen ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 bg-black/78 backdrop-blur-[1.5px]">
          <div className="relative flex h-full w-full flex-col">
            ...
            <div className="relative min-h-0 flex-1 px-4 pb-24 sm:px-8">
              {showLeftSpeaker ? <motion.div initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} className="pointer-events-none absolute bottom-0 left-0 h-[72%] w-[52%]"><Image src={activePortraitUrl} alt={`Retrato de ${activeSpeaker}`} fill sizes="(max-width: 768px) 100vw, 52vw" quality={55} className="object-contain object-bottom-left drop-shadow-[0_0_22px_rgba(34,211,238,0.25)]" /></motion.div> : null}
              {showRightSpeaker ? <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} className="pointer-events-none absolute bottom-0 right-0 h-[72%] w-[52%]"><Image src={activePortraitUrl} alt={`Retrato de ${activeSpeaker}`} fill sizes="(max-width: 768px) 100vw, 52vw" quality={55} className="object-contain object-bottom-right drop-shadow-[0_0_22px_rgba(34,211,238,0.25)]" /></motion.div> : null}
              <motion.article initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 14 }} className={cn("pointer-events-auto absolute bottom-24 z-20 w-[min(92vw,640px)] rounded-xl border border-cyan-400/45 bg-slate-950/97 p-5 shadow-[0_0_30px_rgba(6,182,212,0.25)]", activeSide === "RIGHT" ? "right-4 sm:right-8" : "left-4 sm:left-8")}>
                <p className="text-sm font-black uppercase tracking-[0.14em] text-cyan-200 drop-shadow-[0_0_10px_rgba(34,211,238,0.25)]">{line?.speaker ?? "Sistema"}</p>
                <p className="mt-2 text-base font-semibold leading-relaxed text-white sm:text-lg [text-shadow:0_2px_10px_rgba(0,0,0,0.7)]">{line?.text ?? "Sin contenido narrativo."}</p>
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-5 z-30 flex items-center justify-center gap-2 px-4">
              <button type="button" onClick={onClose} className="pointer-events-auto rounded border border-slate-700 bg-black/70 px-3 py-2 text-xs font-black uppercase tracking-wider text-slate-200">Cerrar</button>
              <button type="button" onClick={onNext} className="pointer-events-auto rounded-full border border-cyan-400/70 bg-cyan-950/80 px-5 py-3 text-xs font-black uppercase tracking-[0.18em] text-cyan-100 shadow-[0_0_20px_rgba(34,211,238,0.3)]">Siguiente diálogo</button>
            </div>
