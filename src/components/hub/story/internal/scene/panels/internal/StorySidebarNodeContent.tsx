...
export function StorySidebarNodeContent(props: IStorySidebarNodeContentProps) {
  return (
    <motion.div key={props.selectedNode.id} variants={storySidebarContainerVariants} initial="hidden" animate="visible" exit="exit" className="flex h-full flex-col">
      <motion.div variants={storySidebarItemVariants} className="mb-4 flex items-start justify-between">
      ...
      <motion.h3 variants={storySidebarItemVariants} className="mb-2 text-xl font-black uppercase tracking-[0.08em] text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
        {props.viewModel.nodeTitle}
      </motion.h3>
      <motion.div variants={storySidebarItemVariants}>
        {props.viewModel.nodeOpponent ? (
          <p className="mb-6 flex items-center text-sm font-bold uppercase tracking-widest text-fuchsia-300 drop-shadow-[0_0_8px_rgba(232,121,249,0.5)]"><span className="mr-2 animate-pulse">⚠️</span> HOSTIL: {props.viewModel.nodeOpponent}</p>
        ) : (
          <p className="mb-6 flex items-center text-xs font-bold uppercase tracking-[0.2em] text-emerald-400"><span className="mr-2">✓</span> Tránsito Seguro</p>
        )}
      </motion.div>

      <motion.div variants={storySidebarItemVariants} className="mb-auto space-y-3 rounded-md border border-white/5 bg-black/40 p-4">
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <span className="text-sm font-black uppercase tracking-[0.15em] text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.4)]">+ {props.viewModel.nodeReward} NX</span>
        </div>
      </motion.div>

      <motion.div variants={storySidebarItemVariants} className="mt-8 space-y-3">
        <button type="button" onClick={props.onSmartAction} disabled={!props.canRunSmartAction || props.isMoving} className={cn("relative w-full overflow-hidden px-4 py-4 text-sm font-black uppercase tracking-[0.2em] transition-all active:scale-[0.98] before:absolute before:inset-0 before:-z-10 before:bg-gradient-to-r [clip-path:polygon(0_0,100%_0,100%_calc(100%-12px),calc(100%-12px)_100%,0_100%)]", props.selectedNode.nodeType === "BOSS" ? "border-l-2 border-t-2 border-fuchsia-400 text-white before:from-fuchsia-600/60 before:to-fuchsia-900/60 hover:before:from-fuchsia-500/80 hover:shadow-[0_0_30px_rgba(217,70,239,0.6)]" : "border-l-2 border-t-2 border-cyan-400 text-white before:from-cyan-600/60 before:to-cyan-900/60 hover:before:from-cyan-500/80 hover:shadow-[0_0_30px_rgba(34,211,238,0.6)]", (!props.canRunSmartAction || props.isMoving) && "cursor-not-allowed grayscale opacity-60 hover:shadow-none")}>
