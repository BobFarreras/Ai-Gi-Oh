...
export function MarketRevealCardsGrid({ cards, onClose }: MarketRevealCardsGridProps) {
  return (
    <motion.div
      ...
      className="relative z-50 flex h-full w-full max-w-7xl flex-col items-center justify-center px-4"
    >
      <motion.h2
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.4 }}
        className="mb-10 text-center text-xl font-black uppercase tracking-[0.4em] text-cyan-300 drop-shadow-[0_0_15px_rgba(34,211,238,0.6)] sm:text-3xl"
      >
        Datos Desencriptados
      </motion.h2>

      <div className="perspective-[1000px] flex flex-wrap items-center justify-center gap-4 sm:gap-6 lg:gap-8">
        {cards.map((card, index) => (
          ...
          >
            <div className="relative h-[180px] w-[140px] drop-shadow-[0_20px_40px_rgba(0,0,0,0.8)] sm:h-[220px] sm:w-[170px] xl:h-[260px] xl:w-[200px]">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: cards.length * 0.35 + 0.8 }} className="mt-14">
        <button
          type="button"
          onClick={onClose}
