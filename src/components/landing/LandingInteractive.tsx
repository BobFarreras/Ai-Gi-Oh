// src/components/landing/LandingInteractive.tsx - Isla interactiva de la landing (intro terminal → narrativa →
// showcase) con persistencia de intro y audio contextual. Se hidrata en cliente; el contenido SEO (h1,
// descripción, CTAs) lo renderiza el Server Component `app/page.tsx` para que los crawlers lo vean sin JS.
"use client";
            ...
            {phase === "SHOWCASE" ? (
              <motion.div key="showcase" variants={containerVariants} initial="hidden" animate="visible" className="absolute inset-0 z-30 flex h-dvh flex-col items-center justify-between overflow-x-visible overflow-y-auto px-4 py-8 pointer-events-auto md:overflow-x-hidden sm:px-6">
                <motion.header variants={itemVariants} className="relative mt-2 flex-shrink-0 text-center">
                  {/* El h1 semántico lo renderiza el Server Component (sr-only) para SEO; este título es
                      decorativo (div), evitando dos h1 en la página. */}
                  <div className="bg-gradient-to-br from-white via-cyan-100 to-cyan-600 bg-clip-text text-4xl font-black uppercase tracking-tighter text-transparent drop-shadow-[0_0_15px_rgba(6,182,212,0.6)] md:text-5xl lg:text-6xl">AI-GI-OH</div>
