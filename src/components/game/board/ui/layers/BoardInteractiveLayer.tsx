...
export function BoardInteractiveLayer({
  ...
  onActivateSelectedExecution, onSelectCard, onCloseCard, onCloseHistory,
}: BoardInteractiveLayerProps) {
  const player = gameState.playerA;
  const opponent = gameState.playerB;
  const viewport = useBoardViewportScale({ hasLeftPanel: Boolean(selectedCard), hasRightPanel: isHistoryOpen });
  ...
  return (
    <>
      ...
      <PlayerHand
        hand={player.hand} playingCard={playingCard} hasSummoned={gameState.hasNormalSummonedThisTurn} isPlayerTurn={isPlayerTurn}
        highlightedCardIds={pendingDiscardCardIds}
        cardScale={viewport.handCardScale} overlapPx={viewport.handOverlapPx} handYOffsetPx={viewport.handYOffsetPx} containerHeightPx={viewport.handContainerHeightPx}
        hoverLiftPx={viewport.handHoverLiftPx}
        centerOffsetPx={viewport.handCenterOffsetPx}
        onMandatoryCardSelect={onMandatoryCardSelect} onCardClick={onCardClick} onPlayAction={onPlayAction}
      />
      <div onClick={(event) => event.stopPropagation()}>
