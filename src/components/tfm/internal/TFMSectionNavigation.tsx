  ...
  return (
    <nav aria-label="Navegación de la presentación" className="rounded-xl border border-cyan-500/35 bg-black/60 p-3 backdrop-blur">
      <ul className="flex flex-wrap gap-2">
        {links.map((link) => (
          <li key={link.id}>
            <a href={`#${link.id}`} onClick={(event) => handleSmoothScroll(event, link.id)} className="inline-flex rounded-md border border-cyan-500/35 bg-cyan-900/20 px-3 py-2 text-sm font-semibold uppercase tracking-[0.08em] text-cyan-100 transition hover:border-cyan-300 hover:bg-cyan-900/40">
              {link.label}
            </a>
          </li>
        ))}
