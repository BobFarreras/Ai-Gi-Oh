...
export function resolveOpponentEntityToLockSelectionAction(
  state: GameState,
  playerId: string,
  selectedId: string,
  player: IPlayer,
  opponent: IPlayer,
  isPlayerA: boolean,
  pending: ISelectOpponentEntityToLockPendingTurnAction,
): GameState {
  ...
  const updatedOpponent: IPlayer = {
    ...
    activeEntities: opponent.activeEntities.map((entity) =>
      entity.instanceId === selectedId ? { ...entity, lockedTurnsRemaining: pending.turns } : entity,
    ),
  };
