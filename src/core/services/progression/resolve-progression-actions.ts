...
export type DuelProgressionMode = "STORY" | "TRAINING" | "SURVIVAL" | "MULTIPLAYER";

/**
 * Acciones de progresión emitidas al cerrar un duelo. Centralizado para que las rutas
 * de story/training/survival/multiplayer no diverjan y se pueda testear el mapeo completo.
 * Survival se tratará como TRAINING: cada combate suma PLAY_ARENA y victoria suma WIN_ARENA.
 */
export function resolveDuelProgressionActions(mode: DuelProgressionMode, won: boolean, flawless = false): ProgressionActionType[] {
  ...
  if (mode === "TRAINING" || mode === "SURVIVAL") actions.push("PLAY_ARENA");
  if (mode === "MULTIPLAYER") actions.push("PLAY_MP_MATCH");
  if (won) {
    actions.push("WIN_DUEL");
    if (mode === "TRAINING" || mode === "SURVIVAL") actions.push("WIN_ARENA");
    ...
    if (flawless) {
      ...
      if (mode === "TRAINING" || mode === "SURVIVAL") actions.push("WIN_FLAWLESS_TRAINING");
